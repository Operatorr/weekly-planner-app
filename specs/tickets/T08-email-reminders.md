...
Implement the email reminder system for Marrow Tasker using Resend as the email service provider. When a user sets a reminder on a task, the system sends a well-designed email at the specified time with the task details and a link back to the app. This ticket covers the Resend integration, email templates, the server-side reminder processing job, and the Vercel Cron configuration.

**References**: __@Epic Brief: Marrow Tasker__ (Task Reminders feature), __@Core Flows: Marrow Tasker__ (F2.1 Set a Task Reminder)
...
1. **Resend Integration** (__@src/server/email/resend.ts__)
   ...
   * Configure sender domain/email (e.g., `reminders@marrowtasker.com` or Resend's default)
...
2. **Email Templates** (__@src/server/email/templates/__)
   * `task-reminder.tsx` — React Email template for task reminders:
     ...
     - CTA button: "Open in Marrow Tasker" (links to the task in the app)
     - Footer: "You're receiving this because you set a reminder in Marrow Tasker" + unsubscribe link (future)
     - Design: clean, minimal, matches Marrow Tasker brand — white background, neutral typography, subtle accent color
   * `welcome.tsx` — Welcome email template (sent after sign-up):
     - Subject: "Welcome to Marrow Tasker"
...
* Task reminder email is well-designed and matches Marrow Tasker branding
* Reminder emails contain: task name, description, due date, and a working "Open in Marrow Tasker" link
...
* For email links, use the app's base URL from an environment variable: `APP_URL` (e.g., `https://marrowtasker.com/app/task/{taskId}`)
